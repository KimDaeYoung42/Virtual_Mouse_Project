...
class MouseFunction:
    ...
    # 마우스 이동 이밴트 처리
    def handle_mouse_move(self, lm_list):
        if MouseFunction.active_stop:
            return

        cursor_pos = lm_list[9]
    ...
    def handle_left_mouse_press(self, event, lm_list):
    ...
    # 마우스 Drag And Drop 이벤트 처리 - 기능 추가 필요!
    def handle_drag_and_drop(self, event):
        self.text_view.append('드래그 앤 드롭 이벤트 감지')
        if MouseFunction.active_stop:
            return
    ...
    def handle_mouse_scroll(self, event):
        ...
        if delta > 0:
            self.handle_mouse_zoom_in()
        elif delta < 0:
            self.handle_mouse_zoom_out()
